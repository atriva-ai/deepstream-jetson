...
```bash
./scripts/download_peoplenet.sh   # NGC_API_KEY required — get from ngc.nvidia.com
./scripts/download_reid.sh        # downloads ResNet50 ReID ONNX
```

Models go in `core_engine/models/`. TRT engine files are auto-generated by `nvinfer`
on first container start (~5 min on Jetson Orin NX) and cached in the
`deepstream_engine_cache` Docker named volume — subsequent starts are fast.
